...
if (process.argv.includes("--list-models")) {
  ...
  const kept =
    filter && !filter.startsWith("-")
      ? [rows[0], ...rows.slice(1).filter((r) => r.includes(filter))]
      : rows;
...
async function runAgent() {
  ...
  if (mode === "empty_final") {
    ...
    out({
      type: "message_end",
      message: {
        role: "assistant", provider: "fake", model: "fake-1", stopReason: "toolUse",
    ...
    out({
      type: "message_end",
      message: {
        role: "assistant", provider: "fake", model: "fake-1", stopReason: "stop",
  ...
  if (mode === "no_stop") {
    ...
    out({
      type: "message_end",
      message: {
        role: "assistant", provider: "fake", model: "fake-1",
